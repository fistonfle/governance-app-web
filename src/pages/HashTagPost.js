...
import { fetchPostsByHashtag } from "../features/postsSlice"; // Action to fetch posts by hashtag
import { useParams, Link } from "react-router-dom";
import { FaThumbsUp } from "react-icons/fa";
import Skeleton from "react-loading-skeleton"; // Import Skeleton loader

const HashtagPage = () => {
  const { hashtag } = useParams(); // Access the hashtag from URL
  const dispatch = useDispatch();
  const { posts, status, error } = useSelector((state) => state.posts);

  useEffect(() => {
    // Dispatch action to fetch posts related to the hashtag
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-8">Posts related to #{hashtag}</h1>
      {/* Posts Grid */}
